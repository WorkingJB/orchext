...
pub const SERVER_NAME: &str = "orchext";
...
impl Server {
        ...
        match parsed {
            ...
            resources::Parsed::Type(type_) => {
                ...
                for entry in entries {
                    if let Ok(doc) = self.vault.read(&entry.id).await {
                        if is_visible(&doc.frontmatter.visibility, &allowed) {
                            lines.push(format!("orchext://vault/{}/{}", type_, entry.id));
                        }
                    }
                }
                let text = lines.join("\n");
    ...
    /// (or a broader subscribed prefix like `orchext://vault/<type>/`) is
...
/// `orchext://vault/<type>/<id>` matches only that exact URI; a trailing-
/// slash form like `orchext://vault/<type>/` matches any document of that
/// type; `orchext://vault/` (or `orchext://vault`) matches everything.
fn uri_matches_sub(uri: &str, sub: &str) -> bool {
    if sub == uri {
        return true;
    }
    if sub == "orchext://vault/" || sub == "orchext://vault" {
        return uri.starts_with("orchext://vault/");
    }
