...
/// Teams admin / browser pane (Phase 3 platform Slice 2).
///
/// Available to every org member: non-admins see the team list and
/// can dive into any team's member roster. Org admins/owners get the
/// "New team" affordance, rename/delete on hover, and add/remove
/// members on the detail panel; team managers get add/remove on the
/// teams they manage.
export function TeamsView({ ctx }: { ctx: Context & { kind: "org" } }) {
  ...
  async function reload() {
    setError(null);
    try {
      const r = await api.teamsList(ctx.orgId);
      setTeams(r.teams);
  ...
  async function createTeam(e: React.FormEvent) {
    ...
    setError(null);
    try {
      const created = await api.teamCreate(ctx.orgId, trimmed);
      setNewName("");
      setShowNew(false);
      await reload();
  ...
  async function deleteTeam(teamId: string) {
    ...
    setError(null);
    try {
      await api.teamDelete(ctx.orgId, teamId);
      await reload();
    } catch (e) {
      setError(e instanceof ApiFailure ? e.message : String(e));
    } finally {
...
function TeamDetail({
    ...
    (async () => {
      try {
        const [m, om] = await Promise.all([
          api.teamMembers(ctx.orgId, team.id),
          isOrgAdmin
  ...
  async function add(e: React.FormEvent) {
    ...
    setError(null);
    try {
      await api.teamMemberAdd(ctx.orgId, team!.id, addAccountId, addRole);
      const m = await api.teamMembers(ctx.orgId, team!.id);
      setMembers(m.members);
      setAddAccountId("");
      onChanged();
    } catch (e) {
  ...
  async function changeRole(
    ...
    setError(null);
    try {
      await api.teamMemberUpdate(ctx.orgId, team!.id, accountId, role);
      const m = await api.teamMembers(ctx.orgId, team!.id);
      setMembers(m.members);
    } catch (e) {
      setError(e instanceof ApiFailure ? e.message : String(e));
  ...
  async function remove(accountId: string) {
    ...
    setError(null);
    try {
      await api.teamMemberRemove(ctx.orgId, team!.id, accountId);
      const m = await api.teamMembers(ctx.orgId, team!.id);
      setMembers(m.members);
      onChanged();
    } catch (e) {
  ...
  async function rename(e: React.FormEvent) {
    ...
    setError(null);
    try {
      await api.teamUpdate(ctx.orgId, team!.id, { name: trimmed });
      setRenameOpen(false);
      onChanged();
    } catch (e) {
      setError(e instanceof ApiFailure ? e.message : String(e));
  ...
  // Build the "available to add" list = org members who aren't already
  // on this team. Only org admins see the picker; team managers can
  // promote/demote/remove existing team members but not pull new
  // people from the broader org roster (D11).
