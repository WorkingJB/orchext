...
export function ProposalsView({
  ...
  async function refresh() {
    try {
      setError(null);
      const resp = await api.proposalsList(tenant.tenant_id, filter);
  ...
  async function approve(p: Proposal) {
    setBusy(p.id);
    try {
      await api.proposalApprove(tenant.tenant_id, p.id);
      await refresh();
    } catch (e) {
      setError(`approve ${p.id}: ${errMessage(e)}`);
    } finally {
      setBusy(null);
    }
  }
  ...
  async function reject(p: Proposal) {
    setBusy(p.id);
    try {
      await api.proposalReject(tenant.tenant_id, p.id);
      await refresh();
    } catch (e) {
      setError(`reject ${p.id}: ${errMessage(e)}`);
    } finally {
      setBusy(null);
    }
  }
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      {docFocus && (
        <div className="mb-4 px-3 py-2 bg-amber-50 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-800 rounded-md flex items-center justify-between gap-3">
