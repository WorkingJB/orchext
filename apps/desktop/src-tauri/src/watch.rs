...
//!
//! Mirrors the pattern in `crates/mytex-mcp/src/watch.rs`: a sync thread
...
use mytex_vault::{DocumentId, VaultDriver};
...
/// Shape must be `<root>/<type>/<id>.md`; `.mytex/`, dot-dirs, and deeper
...
mod tests {
    ...
    fn skips_dot_mytex() {
        let root = Path::new("/vault");
        assert!(classify(root, Path::new("/vault/.mytex/audit.jsonl")).is_none());
    }
