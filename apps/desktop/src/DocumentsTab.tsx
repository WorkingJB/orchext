...
export function DocumentsTab({
  ...
  // Reset subtab on context switch so a per-context proposals view
    ...
    <div className="h-full flex flex-col min-h-0">
      <div className="border-b border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 px-4 flex items-center gap-1">
        ...
        <SubTabBtn
          ...
          onClick={() => {
            setSubtab("proposals");
            // Clearing the focus on a header click lets the user
            // browse all pending proposals from here.
      ...
      <div className="flex-1 min-h-0">
        {subtab === "documents" && (
          <DocumentsView
            ctx={ctx}
            onMutated={onMutated}
            onSwitchToProposals={(docId) => onSetProposalsFocus(docId)}
          />
        )}
        {subtab === "proposals" && (
          <ProposalsView
