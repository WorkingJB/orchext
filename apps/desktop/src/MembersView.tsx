...
/// caller's role is admin/owner — Layout gates the nav button.
///
...
export function MembersView({ ctx }: { ctx: Context & { kind: "org" } }) {
  ...
  useEffect(() => {
    api
      .authMe(ctx.workspaceId)
      .then((r) => setMyAccountId(r.account.id))
      .catch(() => {});
  }, [ctx.workspaceId]);
  ...
  async function reload() {
    ...
    try {
      const [m, p, i] = await Promise.all([
        ...
        api.orgPending(ctx.workspaceId, ctx.orgId, "pending"),
        api.orgInvitations(ctx.workspaceId, ctx.orgId, "open"),
      ]);
  ...
  async function changeRole(memberId: string, newRole: Role) {
    ...
    setError(null);
    try {
      await api.orgMemberUpdate(ctx.workspaceId, ctx.orgId, memberId, newRole);
      await reload();
    } catch (e) {
      setError(String(e));
    } finally {
  ...
  async function remove(memberId: string) {
    ...
    setError(null);
    try {
      await api.orgMemberRemove(ctx.workspaceId, ctx.orgId, memberId);
      await reload();
    } catch (e) {
      setError(String(e));
    } finally {
  ...
  async function approve(accountId: string, role: Role) {
    ...
    setError(null);
    try {
      await api.orgPendingApprove(ctx.workspaceId, ctx.orgId, accountId, role);
      await reload();
    } catch (e) {
      setError(String(e));
    } finally {
  ...
  async function reject(accountId: string) {
    ...
    setError(null);
    try {
      await api.orgPendingReject(ctx.workspaceId, ctx.orgId, accountId);
      await reload();
    } catch (e) {
      setError(String(e));
    } finally {
  ...
  async function submitInvite() {
    ...
    setError(null);
    try {
      await api.orgInvite(ctx.workspaceId, ctx.orgId, email, inviteRole);
      setInviteEmail("");
  ...
  async function revokeInvitation(id: string) {
    ...
    setError(null);
    try {
      await api.orgInvitationDelete(ctx.workspaceId, ctx.orgId, id);
      await reload();
    } catch (e) {
      setError(String(e));
    } finally {
